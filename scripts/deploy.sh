...
DEFAULT_TARGET="domains/aarohitavigyan.com/public_html"
DEFAULT_PORT="65002"
...
SYNC_SOURCE="$BUILD_DIR"
if [ -d "$BUILD_DIR/dist" ]; then
  SYNC_SOURCE="$BUILD_DIR/dist"
fi

EXCLUDE_NODE_MODULES=${EXCLUDE_NODE_MODULES:-1}

echo "🔁 Syncing files to Hostinger ($DEPLOY_USER@$DEPLOY_HOST:$DEPLOY_TARGET)…"
SOURCE_DIR="$SYNC_SOURCE" RUN_REMOTE_COMMANDS=${RUN_REMOTE_COMMANDS:-0} EXCLUDE_NODE_MODULES=$EXCLUDE_NODE_MODULES HOST=$DEPLOY_HOST USER=$DEPLOY_USER TARGET=$DEPLOY_TARGET PORT=$DEPLOY_PORT SSH_KEY=$SSH_KEY \
